...
function WhatsAppBtn({ phone, message='' }) {
  ...
  return <a href={url} target="_blank" rel="noopener" style={{display:'flex',alignItems:'center',gap:8,background:'#25D366',color:'#fff',padding:'10px 20px',borderRadius:4,textDecoration:'none',fontWeight:800,fontSize:14,letterSpacing:1,fontFamily:"'Barlow Condensed',sans-serif"}}>
...
function BrandStrip() {
  return <div style={{background:'#111',borderTop:'1px solid #1e1e1e',borderBottom:'1px solid #1e1e1e',padding:'14px 20px'}}>
    <div style={{maxWidth:1360,margin:'0 auto'}}>
      <div style={{fontSize:9,color:'#444',letterSpacing:3,textAlign:'center',marginBottom:10}}>AUTHORISED STOCKIST</div>
      <div style={{display:'flex',gap:24,justifyContent:'center',flexWrap:'wrap'}}>
        {BRANDS.map(b=><div key={b} style={{color:'#2a2a2a',fontSize:13,fontWeight:800,letterSpacing:2,fontFamily:"'Barlow Condensed',sans-serif",textTransform:'uppercase',transition:'color .15s',cursor:'default'}} onMouseEnter={e=>e.target.style.color=O} onMouseLeave={e=>e.target.style.color='#2a2a2a'}>{b}</div>)}
      </div>
    </div>
  </div>
}
...
function Navbar({ page, setPage, cartCount, onCartOpen, settings:s }) {
  return <nav style={{background:'#111',borderBottom:`3px solid ${O}`,position:'sticky',top:0,zIndex:900,boxShadow:'0 4px 24px #FF6B0022'}}>
    <div style={{maxWidth:1360,margin:'0 auto',padding:'0 16px',display:'flex',alignItems:'center',height:62,gap:12}}>
      {s?.hero_logo_url
        ...
        : <svg width="36" height="36" viewBox="0 0 36 36" style={{cursor:'pointer',flexShrink:0}} onClick={()=>setPage('shop')}><circle cx="18" cy="18" r="17" fill={O} stroke="#000" strokeWidth="1.5"/><text x="18" y="24" textAnchor="middle" fontSize="18" fontFamily="serif">🐯</text></svg>
      }
...
function Hero({ settings:s, onBook }) {
  return <div style={{background:'linear-gradient(135deg,#111 0%,#191919 55%,#0d0d0d 100%)',padding:'60px 20px 48px',textAlign:'center',position:'relative',overflow:'hidden'}}>
    ...
    <div style={{position:'relative',maxWidth:900,margin:'0 auto'}}>
      {s?.hero_logo_url&&<div style={{marginBottom:22}}><img src={s.hero_logo_url} alt={s.shop_name} style={{maxHeight:110,maxWidth:320,objectFit:'contain'}}/></div>}
      ...
      {s?.price_match==='true'&&s?.hero_badge_text&&<div style={{marginTop:20,display:'inline-flex',alignItems:'center',gap:8,background:'#1a1a1a',border:`1px solid ${O}33`,borderRadius:4,padding:'8px 18px'}}><span>💰</span><span style={{fontSize:12,color:'#aaa',letterSpacing:1}}>{s.hero_badge_text}</span></div>}
...
function SearchPanel({ onVehicleResults, onSizeSearch, showToast }) {
  ...
  useEffect(()=>{
    api.getVehicleMakes().then(setMakes).catch(()=>{})
    api.getWidths().then(setWidths).catch(()=>{})
    api.getProfiles().then(setProfiles).catch(()=>{})
    api.getRims().then(setRims).catch(()=>{})
  },[])
  useEffect(()=>{
  ...
  const searchVehicle = async () => {
    setVLoading(true)
    try { onVehicleResults(await api.getVehicleFitment(sel.make,sel.model,sel.year)) }
  ...
  const tabBtn=(t,label)=>(
    <button onClick={()=>setTab(t)} style={{flex:1,padding:'11px 8px',border:'none',cursor:'pointer',background:tab===t?O:'#1a1a1a',color:tab===t?'#000':'#666',fontWeight:800,fontSize:13,letterSpacing:2,fontFamily:"'Barlow Condensed',sans-serif",borderRadius:t==='vehicle'?'4px 0 0 0':'0 4px 0 0',transition:'all .15s'}}>{label}</button>
  )

  return <div style={{background:'linear-gradient(135deg,#161616,#1a1a1a)',border:`1px solid ${O}33`,borderRadius:6,overflow:'hidden'}}>
    <div style={{display:'flex',borderBottom:`2px solid ${O}22`}}>
      {tabBtn('vehicle','🚗  SEARCH BY VEHICLE')}
      {tabBtn('size','🔍  SEARCH BY TYRE SIZE')}
    </div>
    <div style={{padding:'20px 22px'}}>
      {tab==='vehicle'&&<>
        <div style={{color:'#555',fontSize:12,marginBottom:14}}>Select your vehicle and we'll show every compatible tyre we have in stock.</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:12,alignItems:'end'}}>
          ...
          <Btn onClick={searchVehicle} disabled={!sel.year||vLoading} style={{whiteSpace:'nowrap'}}>{vLoading?'SEARCHING…':'🔍 FIND TYRES'}</Btn>
        </div>
      </>}
      {tab==='size'&&<>
        <div style={{color:'#555',fontSize:12,marginBottom:14}}>
          Read the size off your tyre sidewall — e.g. <span style={{color:O,fontFamily:'monospace',fontWeight:700}}>225/45 R18</span> — then search below.
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(135px,1fr))',gap:12,alignItems:'end'}}>
          <div><label style={lbl}>Width (mm)</label>
            <select value={size.width} onChange={e=>setSize(s=>({...s,width:e.target.value}))} style={inp}>
              <option value="">Any width</option>{widths.map(w=><option key={w} value={w}>{w}</option>)}
            </select>
          </div>
          <div><label style={lbl}>Profile (%)</label>
            <select value={size.profile} onChange={e=>setSize(s=>({...s,profile:e.target.value}))} style={inp}>
              <option value="">Any profile</option>{profiles.map(p=><option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div><label style={lbl}>Rim (inch)</label>
            <select value={size.rim} onChange={e=>setSize(s=>({...s,rim:e.target.value}))} style={inp}>
              <option value="">Any rim</option>{rims.map(r=><option key={r} value={r}>{r}"</option>)}
            </select>
          </div>
          <div><label style={lbl}>Condition</label>
            <select value={size.condition} onChange={e=>setSize(s=>({...s,condition:e.target.value}))} style={inp}>
              <option value="">New or Used</option>
              <option value="New">New only</option>
              <option value="Used">Used only</option>
            </select>
          </div>
          <Btn onClick={searchSize} style={{whiteSpace:'nowrap'}}>🔍 SEARCH</Btn>
        </div>
        {(size.width||size.profile||size.rim)&&<div style={{marginTop:12,fontSize:13,color:'#666'}}>
          Searching: <span style={{...BB,fontSize:17,color:O}}>{size.width||'?'}/{size.profile||'?'}R{size.rim||'?'}</span>
          {size.condition&&<span style={{marginLeft:8,color:'#555'}}>· {size.condition} only</span>}
        </div>}
      </>}
    </div>
  </div>
...
function BookingWidget({ onClose, showToast }) {
  ...
  const checkSlots=async()=>{
    ...
    try {
      ...
      if(data.length===0){ showToast('Shop is closed on this date. Please choose another.',false); setLoading(false); return }
      setSlots(data); setStep('slots')
  ...
  return <div style={{minWidth:300,maxWidth:500}}>
    <div style={{...BB,fontSize:24,letterSpacing:3,color:O,marginBottom:20}}>{step==='done'?'✅ BOOKING CONFIRMED!':'BOOK A FITTING'}</div>

    {step==='form'&&<div>
      {[['Name *','name','text'],['Phone *','phone','tel'],['Email (for confirmation)','email','email'],['Vehicle (e.g. 2022 Toyota RAV4)','vehicle','text']].map(([label,key,type])=>(
      ...
      <div style={{marginBottom:14}}><label style={lbl}>Preferred Date</label>
        <input type="date" value={form.date} min={minDate} onChange={e=>f('date',e.target.value)} style={inp}/>
      </div>
      <div style={{marginBottom:20}}><label style={lbl}>Notes</label>
        <textarea value={form.notes} onChange={e=>f('notes',e.target.value)} rows={2} style={{...inp,resize:'vertical'}} placeholder="Tyre sizes, special requests…"/>
      </div>
      {form.email&&<div style={{background:'#1a1a1a',border:`1px solid ${O}22`,borderRadius:4,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#666'}}>
        📧 A confirmation email will be sent to <strong style={{color:'#aaa'}}>{form.email}</strong>
      </div>}
    ...
    {step==='slots'&&<div>
      <div style={{color:'#888',fontSize:13,marginBottom:14}}>Available times on <span style={{color:O,fontWeight:700}}>{form.date}</span></div>
    ...
    {step==='confirm'&&<div>
      ...
      {form.email&&<div style={{background:'#1a2a1a',border:'1px solid #2a4a2a',borderRadius:4,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#4a9'}}>
        📧 Confirmation will be emailed to {form.email}
      </div>}
      <div style={{display:'flex',gap:10}}>
    ...
    {step==='done'&&<div style={{textAlign:'center',padding:'10px 0'}}>
      ...
      <p style={{color:'#aaa',lineHeight:1.8,marginBottom:20}}>
        ...
        {form.email&&<><br/><span style={{fontSize:12,color:'#4a9'}}>📧 Confirmation sent to {form.email}</span></>}
      </p>
...
function ReviewsSection({ tyreId, reviews=[], avgRating, reviewCount, showToast }) {
  ...
  return <div style={{padding:'24px 28px',borderTop:'1px solid #1e1e1e'}}>
    ...
    {reviews.map(r=>(
      <div key={r.id} style={{background:'#1a1a1a',border:'1px solid #222',borderRadius:4,padding:'12px 16px',marginBottom:10}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><div style={{fontWeight:700,fontSize:14}}>{r.author}</div><Stars rating={r.rating} size={13}/></div>
    ...
    {!submitted?<div style={{background:'#161616',border:'1px solid #2a2a2a',borderRadius:4,padding:18,marginTop:14}}>
      ...
      <div style={{marginBottom:12}}><label style={lbl}>Rating</label>
        <div style={{display:'flex',gap:4}}>
          {[1,2,3,4,5].map(i=><span key={i} onClick={()=>setForm(f=>({...f,rating:i}))} onMouseEnter={()=>setHovRating(i)} onMouseLeave={()=>setHovRating(0)} style={{fontSize:28,cursor:'pointer',color:i<=(hovRating||form.rating)?O:'#333',transition:'color .1s'}}>★</span>)}
        </div>
      </div>
    ...
    </div>:<div style={{color:'#44dd88',fontSize:13,marginTop:14}}>✓ Thanks! Your review is pending approval.</div>}
  </div>
}
...
function ComparePanel({ tyres, onRemove, onClose }) {
  ...
  return <div style={{position:'fixed',bottom:0,left:0,right:0,background:'#141414',borderTop:`3px solid ${O}`,zIndex:800,padding:'14px 20px',boxShadow:'0 -8px 40px #000a'}}>
    <div style={{maxWidth:1360,margin:'0 auto'}}>
...
function DetailView({ tyre, onBack, onAddCart, showToast }) {
  ...
  return <div style={{maxWidth:1000,margin:'40px auto',padding:'0 20px'}}>
...
function CartDrawer({ cart, onRemove, onClose, showToast }) {
  ...
  const [form,setForm]=useState({name:'',phone:'',email:'',message:'',tyre_size:''})
  ...
  const submit=async()=>{
    setSending(true)
    try {
      await api.submitEnquiry({...form,items:cart.length>0?cart:undefined})
      setStep('done')
    } catch(e) { showToast(e.message,false) }
  ...
  return <>
    ...
    <div style={{position:'fixed',right:0,top:0,bottom:0,width:'min(440px,100vw)',background:'#141414',borderLeft:`2px solid ${O}`,zIndex:1200,display:'flex',flexDirection:'column',boxShadow:'-10px 0 50px #000'}}>
      ...
      {step==='enquire'&&<div style={{flex:1,overflowY:'auto',padding:20}}>
        {[['Name *','name','text'],['Phone *','phone','tel'],['Email (for confirmation)','email','email']].map(([label,key,type])=>(
          <div key={key} style={{marginBottom:14}}><label style={lbl}>{label}</label><input type={type} value={form[key]} onChange={e=>f(key,e.target.value)} style={inp}/></div>
        ))}
        {cart.length===0&&<div style={{marginBottom:14}}>
          <label style={lbl}>Tyre Size You're After (e.g. 225/45 R18)</label>
          <input value={form.tyre_size} onChange={e=>f('tyre_size',e.target.value)} style={inp} placeholder="225/45 R18"/>
        </div>}
        <label style={lbl}>Message</label>
        <textarea value={form.message} onChange={e=>f('message',e.target.value)} rows={3} style={{...inp,resize:'vertical',marginBottom:16}} placeholder="Vehicle, tyre condition preference, budget, etc."/>
        {cart.length>0&&<div style={{background:'#1a1a1a',borderRadius:4,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#666'}}>{cart.length} tyre{cart.length!==1?'s':''} · Total ${total}</div>}
        {form.email&&<div style={{background:'#1a2a1a',border:'1px solid #2a4a2a',borderRadius:4,padding:'8px 12px',marginBottom:14,fontSize:12,color:'#4a9'}}>📧 Confirmation will be emailed to {form.email}</div>}
      ...
      {step==='done'&&<div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:32,textAlign:'center'}}>
        ...
        {form.email&&<p style={{color:'#4a9',fontSize:12,marginTop:8}}>📧 Confirmation sent to {form.email}</p>}
...
function AboutPage({ settings:s, onBook, showToast }) {
  ...
  return <div style={{maxWidth:1360,margin:'0 auto',padding:'40px 20px'}}>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:28}}>
      <div>
        ...
        {s?.price_match==='true'&&<div style={{background:'#1a1a1a',border:`1px solid ${O}44`,borderRadius:4,padding:'14px 18px',display:'flex',alignItems:'center',gap:12}}>
      ...
      <div>
        ...
        {s?.maps_embed?<div style={{borderRadius:6,overflow:'hidden',border:'1px solid #222'}} dangerouslySetInnerHTML={{__html:s.maps_embed}}/>
          :<div style={{background:'#111',border:'1px solid #1a1a1a',borderRadius:6,height:180,display:'flex',alignItems:'center',justifyContent:'center',color:'#333',fontSize:13}}>📍 Add a Google Maps embed in Admin → Settings</div>}
      </div>
      <div>
        <div style={{...BB,fontSize:24,letterSpacing:3,color:O,marginBottom:16}}>SEND A MESSAGE</div>
        {sent?<div style={{textAlign:'center',padding:40}}><div style={{fontSize:48}}>🐯</div><div style={{...BB,fontSize:22,color:O,marginTop:12}}>MESSAGE SENT!</div><p style={{color:'#888',marginTop:10}}>We'll get back to you shortly.</p></div>
        :<>
          {[['Name *','name','text'],['Phone *','phone','tel'],['Email','email','email']].map(([label,key,type])=>(
            <div key={key} style={{marginBottom:14}}><label style={lbl}>{label}</label><input type={type} value={form[key]} onChange={e=>f(key,e.target.value)} style={inp}/></div>
          ))}
          <div style={{marginBottom:14}}><label style={lbl}>Tyre Size (optional, e.g. 225/45 R18)</label><input value={form.tyre_size} onChange={e=>f('tyre_size',e.target.value)} style={inp} placeholder="225/45 R18"/></div>
...
function Footer({ settings:s, onBook }) {
  return <footer style={{background:'#0a0a0a',borderTop:`3px solid ${O}`,marginTop:60,padding:'40px 20px 24px'}}>
    <div style={{maxWidth:1360,margin:'0 auto'}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:28,marginBottom:28}}>
        ...
        <div>
          ...
          {[['New Tyres'],['Used Tyres'],['Book a Fitting',null,onBook],['About Us']].map(([label,,action])=>(
            <div key={label} style={{marginBottom:6}}><span onClick={action} style={{color:'#555',fontSize:13,cursor:'pointer'}} onMouseEnter={e=>e.target.style.color=O} onMouseLeave={e=>e.target.style.color='#555'}>{label}</span></div>
          ))}
        </div>
      </div>
...
function TyreForm({ item, onSave, onCancel, showToast }) {
  ...
  return <div style={{background:'#161616',border:`1px solid ${O}44`,borderRadius:6,padding:28,marginBottom:28}}>
...
function BulkPricePanel({ showToast, onDone }) {
  ...
  return <div style={{background:'#161616',border:`1px solid ${O}44`,borderRadius:6,padding:24,marginBottom:24,maxWidth:500}}>
...
function EnquiriesTab({ showToast }) {
  ...
  return <div>
    ...
    {enquiries.map(e=>{
      ...
      return <div key={e.id} style={{background:'#161616',border:`1px solid ${e.status==='new'?O+'44':'#222'}`,borderRadius:6,marginBottom:10,overflow:'hidden'}}>
        ...
        {expanded===e.id&&<div style={{padding:'0 16px 14px',borderTop:'1px solid #1e1e1e'}}>
          ...
          {items.length>0&&<div style={{marginBottom:12}}>{items.map((it,i)=>(
            <div key={i} style={{display:'flex',justifyContent:'space-between',background:'#1a1a1a',padding:'7px 12px',borderRadius:3,marginBottom:3,fontSize:13}}>
              <span>{it.size||`${it.brand} ${it.model} ${it.width}/${it.profile}R${it.rim} ×${it.qty}`}</span>
              {it.price&&<span style={{color:O}}>${it.price*it.qty}</span>}
            </div>
          ))}</div>}
...
function BookingsTab({ showToast }) {
  ...
  const statusBg    = { pending:'#2a1a00', confirmed:'#0a2a0a', cancelled:'#2a0a0a', completed:'#0a0a2a' }
  ...
  // Today's bookings for print
  const todayBookings = (byDate[todayISO] || []).filter(b => b.status !== 'cancelled').sort((a,b) => a.time.localeCompare(b.time))

  const printToday = () => {
    const shopName = document.title.replace(' — Tiger Tyres','') || 'Tiger Tyres'
    const rows = todayBookings.map(b => `
      <tr>
        <td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:bold">${b.time}</td>
        ...
        <td style="padding:8px 12px;border-bottom:1px solid #eee">${b.vehicle||'—'}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #eee;text-transform:capitalize;color:${b.status==='confirmed'?'green':b.status==='pending'?'orange':'grey'}">${b.status}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #eee">${b.notes||''}</td>
      </tr>`).join('')

    const html = `<!DOCTYPE html><html><head><title>Bookings – ${todayISO}</title>
    <style>body{font-family:Arial,sans-serif;margin:24px;color:#111}h1{font-size:22px;margin-bottom:4px}h2{font-size:14px;color:#888;font-weight:normal;margin-bottom:20px}table{width:100%;border-collapse:collapse}th{background:#FF6B00;color:#000;padding:10px 12px;text-align:left;font-size:12px;letter-spacing:1px}tr:hover{background:#f9f9f9}@media print{button{display:none}}</style>
    </head><body>
    <h1>🐯 ${shopName} — Daily Bookings</h1>
    <h2>${new Date(todayISO+'T12:00:00').toLocaleDateString('en-NZ',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</h2>
    ${todayBookings.length === 0
      ? '<p style="color:#888">No bookings today.</p>'
      : `<table><thead><tr><th>TIME</th><th>NAME</th><th>PHONE</th><th>SERVICE</th><th>VEHICLE</th><th>STATUS</th><th>NOTES</th></tr></thead><tbody>${rows}</tbody></table>
         <p style="margin-top:20px;color:#888;font-size:12px">Total: ${todayBookings.length} booking${todayBookings.length!==1?'s':''} · Printed ${new Date().toLocaleTimeString('en-NZ')}</p>`
    }
  ...
  const weekLabel = () => {
    ...
    return `${weekStart.toLocaleDateString('en-NZ',opts)} – ${end.toLocaleDateString('en-NZ',{...opts,year:'numeric'})}`
  }
  ...
  return <div>
    ...
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap',gap:12}}>
      <div style={{...BB,fontSize:24,letterSpacing:3,color:O}}>BOOKINGS</div>
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
        <Btn variant="secondary" onClick={prevWeek} style={{padding:'7px 14px',fontSize:13}}>← Prev</Btn>
        <Btn variant="secondary" onClick={goToday}  style={{padding:'7px 14px',fontSize:12}}>Today</Btn>
        <Btn variant="secondary" onClick={nextWeek} style={{padding:'7px 14px',fontSize:13}}>Next →</Btn>
        <span style={{color:'#666',fontSize:13,minWidth:180,textAlign:'center',...BB,letterSpacing:1}}>{weekLabel()}</span>
        <Btn onClick={printToday} style={{padding:'7px 16px',fontSize:12}}>
          🖨️ Print Today{todayBookings.length > 0 ? ` (${todayBookings.length})` : ''}
        </Btn>
      </div>
    </div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:24}}>
      {weekDays.map(day => {
        ...
        const dayNum   = day.toLocaleDateString('en-NZ',{day:'numeric',month:'short'})

        return (
          <div key={iso} style={{
            background: isToday ? '#1a1400' : '#161616',
            border: `1px solid ${isToday ? O : '#222'}`,
            borderRadius:6, minHeight:180, overflow:'hidden',
          }}>
            {/* Day header */}
            <div style={{
              background: isToday ? O : '#1a1a1a',
              padding:'8px 10px',
              borderBottom:`1px solid ${isToday ? '#cc5500' : '#222'}`,
            }}>
              <div style={{...BB,fontSize:14,letterSpacing:2,color:isToday?'#000':'#aaa'}}>{dayName}</div>
              <div style={{fontSize:11,color:isToday?'#333':'#555'}}>{dayNum}</div>
            </div>

            {/* Bookings for this day */}
            <div style={{padding:'6px 4px'}}>
              {dayBookings.length === 0
                ? <div style={{color:'#2a2a2a',fontSize:11,textAlign:'center',padding:'16px 4px'}}>—</div>
                : dayBookings.map(b => (
                  <div key={b.id}
                    onClick={() => setExpanded(expanded===b.id ? null : b.id)}
                    style={{
                      background: statusBg[b.status] || '#1a1a1a',
                  ...
                  >
                    <div style={{...BB,fontSize:13,color:statusColor[b.status],letterSpacing:1}}>{b.time}</div>
                    <div style={{fontSize:12,fontWeight:700,color:'#ddd',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{b.name}</div>
                    <div style={{fontSize:10,color:'#666',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{b.service}</div>
                  </div>
                ))
              }
            </div>
    ...
    {expanded && (() => {
      const b = bookings.find(x => x.id === expanded)
      if (!b) return null
      return (
        <div style={{background:'#161616',border:`1px solid ${O}44`,borderRadius:6,padding:20,marginBottom:20}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:14,flexWrap:'wrap',gap:8}}>
            <div>
              <div style={{...BB,fontSize:20,letterSpacing:2}}>{b.name}</div>
              <div style={{color:O,...BB,fontSize:16}}>{b.date} @ {b.time} — {b.service}</div>
            </div>
            <button onClick={()=>setExpanded(null)} style={{background:'transparent',border:'none',color:'#555',cursor:'pointer',fontSize:20}}>✕</button>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:10,marginBottom:14}}>
            {[['Phone',b.phone],['Email',b.email||'—'],['Vehicle',b.vehicle||'—'],['Notes',b.notes||'—']].map(([k,v])=>(
              <div key={k} style={{background:'#1a1a1a',padding:'8px 12px',borderRadius:4}}>
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            {['pending','confirmed','completed','cancelled'].map(s=>(
              <Btn key={s} variant={b.status===s?'primary':'secondary'}
                onClick={()=>updateBooking(b.id,s)}
                style={{padding:'6px 14px',fontSize:12,textTransform:'capitalize'}}>{s}</Btn>
            ))}
            <Btn variant="danger" onClick={()=>{deleteBooking(b.id);setExpanded(null)}} style={{padding:'6px 14px',fontSize:12,marginLeft:'auto'}}>Delete</Btn>
          </div>
        </div>
      )
    })()}

    {/* Full booking list below calendar — upcoming only */}
    <div style={{...BB,fontSize:16,letterSpacing:3,color:'#444',marginBottom:12}}>ALL UPCOMING BOOKINGS</div>
    {bookings.filter(b=>b.date>=todayISO&&b.status!=='cancelled').length===0
      ? <div style={{color:'#333',textAlign:'center',padding:20,fontSize:13}}>No upcoming bookings</div>
      : bookings
          .filter(b=>b.date>=todayISO&&b.status!=='cancelled')
          .sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time))
          .map(b=>(
          <div key={b.id} style={{background:'#161616',border:`1px solid ${b.status==='pending'?O+'33':'#1e1e1e'}`,borderRadius:4,padding:'10px 14px',marginBottom:6,display:'flex',alignItems:'center',gap:12,flexWrap:'wrap',cursor:'pointer'}}
            onClick={()=>setExpanded(expanded===b.id?null:b.id)}>
            <span style={{...BB,fontSize:15,color:O,minWidth:90}}>{b.date}</span>
            <span style={{...BB,fontSize:15,color:'#aaa',minWidth:55}}>{b.time}</span>
            <span style={{fontWeight:700,fontSize:13}}>{b.name}</span>
            <span style={{color:'#666',fontSize:12}}>{b.service}</span>
            {b.vehicle&&<span style={{color:'#555',fontSize:11}}>{b.vehicle}</span>}
            <span style={{marginLeft:'auto',fontSize:10,letterSpacing:2,textTransform:'uppercase',color:statusColor[b.status],border:`1px solid ${statusColor[b.status]}33`,padding:'2px 8px',borderRadius:2}}>{b.status}</span>
          </div>
        ))
    }
  </div>
...
function BlockedDatesTab({ showToast }) {
  ...
  const add=async()=>{
    ...
    setSaving(true)
    try {
      const created=await api.createBlockedDate(form)
      setBlocked(prev=>[...prev,created].sort((a,b)=>a.date.localeCompare(b.date)))
      setForm({date:'',reason:''})
  ...
  const remove=async(id)=>{
    ...
    showToast('Date unblocked',false)
  }

  return <div>
    <div style={{...BB,fontSize:24,letterSpacing:3,color:O,marginBottom:8}}>BLOCKED DATES</div>
    <div style={{color:'#555',fontSize:13,marginBottom:20}}>Block dates when the shop is closed — public holidays, annual leave, etc. Customers won't be able to book these days.</div>

    <div style={{background:'#161616',border:`1px solid ${O}44`,borderRadius:6,padding:22,marginBottom:24,maxWidth:500}}>
      <div style={{...BB,fontSize:16,letterSpacing:2,color:O,marginBottom:14}}>BLOCK A DATE</div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 2fr',gap:12,marginBottom:14}}>
        <div><label style={lbl}>Date</label><input type="date" value={form.date} min={minDate} onChange={e=>setForm(f=>({...f,date:e.target.value}))} style={inp}/></div>
        <div><label style={lbl}>Reason (optional)</label><input value={form.reason} onChange={e=>setForm(f=>({...f,reason:e.target.value}))} style={inp} placeholder="e.g. Christmas Day, Annual Leave"/></div>
      </div>
    ...
    :blocked.length===0?<div style={{color:'#444',textAlign:'center',padding:40}}>No blocked dates. Shop is open every day per your hours settings.</div>
    :<div style={{background:'#141414',border:'1px solid #222',borderRadius:6,overflow:'hidden'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
        <thead><tr style={{background:'#1a1a1a',borderBottom:`2px solid ${O}`}}>
          {['Date','Day','Reason',''].map(h=><th key={h} style={{padding:'10px 16px',textAlign:'left',fontSize:9,letterSpacing:2,color:O,textTransform:'uppercase'}}>{h}</th>)}
        </tr></thead>
        <tbody>
          {blocked.map((b,i)=>{
            const d=new Date(b.date+'T12:00:00')
            const dayName=d.toLocaleDateString('en-NZ',{weekday:'long'})
            const dateStr=d.toLocaleDateString('en-NZ',{day:'numeric',month:'long',year:'numeric'})
            const isPast=b.date<minDate
            return <tr key={b.id} style={{borderBottom:'1px solid #1e1e1e',background:i%2===0?'#161616':'#141414',opacity:isPast?0.5:1}}>
              <td style={{padding:'10px 16px',color:O,...BB,fontSize:16}}>{dateStr}</td>
              <td style={{padding:'10px 16px',color:'#888',fontSize:13}}>{dayName}</td>
              <td style={{padding:'10px 16px',color:'#aaa',fontSize:13}}>{b.reason||'—'}</td>
              <td style={{padding:'10px 16px',textAlign:'right'}}>
                <Btn variant="danger" onClick={()=>remove(b.id)} style={{padding:'4px 10px',fontSize:11}}>Unblock</Btn>
              </td>
            </tr>
          })}
        </tbody>
    ...
    {reviews.map(r=>(
      <div key={r.id} style={{background:'#161616',border:`1px solid ${r.approved?'#222':O+'44'}`,borderRadius:6,padding:'14px 18px',marginBottom:10,display:'flex',gap:16,flexWrap:'wrap',alignItems:'flex-start'}}>
        <div style={{flex:1}}>
          ...
          {r.body&&<div style={{color:'#aaa',fontSize:13,fontStyle:'italic'}}>"{r.body}"</div>}
          {!r.approved&&<div style={{marginTop:6,fontSize:10,color:O,letterSpacing:2}}>PENDING APPROVAL</div>}
        </div>
...
function VehiclesTab({ showToast }) {
  ...
  const save=async()=>{
    if(!form.make||!form.model||!form.year) return showToast('Make, model and year required',false)
    setSaving(true)
    try {
      if(editItem==='new'){const c=await api.createVehicle(form);setVehicles(prev=>[c,...prev]);showToast(`${form.make} ${form.model} ${form.year} added ✓`)}
      else{const u=await api.updateVehicle(editItem.id,form);setVehicles(prev=>prev.map(v=>v.id===editItem.id?u:v));showToast('Updated ✓')}
      setEditItem(null)
    } catch(e){showToast(e.message,false)}
    finally{setSaving(false)}
  }
  const confirmDelete=async()=>{
    ...
    setDeleteTarget(null); showToast('Deleted',false)
  }
  const doBulk=async()=>{
    ...
    lines.forEach((line,i)=>{
      ...
      if(parts.length<6){errors.push(`Line ${i+1}: need 6 columns (Make, Model, Year, Width, Profile, Rim)`);return}
    ...
    setSaving(true)
    try{const r=await api.bulkVehicles(parsed);showToast(`Imported ${r.added} vehicles`);const fresh=await api.getAdminVehicles();setVehicles(fresh);setShowBulk(false);setBulkText('')}
    catch(e){showToast(e.message,false)}
    finally{setSaving(false)}
  }
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8,flexWrap:'wrap',gap:10}}>
      <div>
        ...
        <div style={{color:'#555',fontSize:12,marginTop:4}}>{vehicles.length} vehicles · controls which tyres show up when customers search by vehicle</div>
      </div>
      <div style={{display:'flex',gap:8}}>
    ...
    <div style={{background:'#1a1a1a',border:`1px solid ${O}22`,borderRadius:6,padding:'12px 16px',marginBottom:20,display:'flex',gap:12,alignItems:'flex-start'}}>
      <span style={{fontSize:20}}>ℹ️</span>
      <div style={{color:'#666',fontSize:13,lineHeight:1.7}}>
        Each entry maps a <strong style={{color:'#ddd'}}>Make + Model + Year</strong> to an OE tyre size.
        When a customer selects their vehicle, we show all in-stock tyres matching that size.
        <span style={{color:O}}> A tyre only appears if its width/profile/rim matches AND qty &gt; 0.</span>
      </div>
    </div>

    {editItem&&<div style={{background:'#161616',border:`1px solid ${O}44`,borderRadius:6,padding:24,marginBottom:20}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:14,marginBottom:16}}>
        {[['Make','make','text','Toyota'],['Model','model','text','RAV4'],['Year','year','number','2024'],['Width (mm)','width','number','235'],['Profile (%)','profile','number','55'],['Rim (in)','rim','number','19']].map(([label,key,type,placeholder])=>(
          <div key={key}><label style={lbl}>{label}</label><input type={type} value={form[key]} onChange={e=>f(key,type==='number'?Number(e.target.value):e.target.value)} style={inp} placeholder={placeholder}/></div>
        ))}
      </div>
      <div style={{marginBottom:14}}><label style={lbl}>Notes (optional)</label><input value={form.notes} onChange={e=>f('notes',e.target.value)} style={inp} placeholder="e.g. GX/GXL trim, 19 inch alloys"/></div>
      <div style={{background:'#111',borderRadius:4,padding:'10px 14px',marginBottom:16,fontSize:13,color:'#888'}}>OE size preview: <span style={{...BB,fontSize:17,color:O}}>{form.width}/{form.profile}R{form.rim}</span></div>
      <div style={{display:'flex',gap:10}}>
    ...
    {showBulk&&<div style={{background:'#161616',border:`1px solid ${O}44`,borderRadius:6,padding:24,marginBottom:20}}>
      <div style={{...BB,fontSize:20,letterSpacing:3,color:O,marginBottom:8}}>BULK IMPORT</div>
      <div style={{color:'#666',fontSize:12,marginBottom:14,lineHeight:1.7}}>One vehicle per line: <code style={{color:O,background:'#111',padding:'2px 6px',borderRadius:3}}>Make, Model, Year, Width, Profile, Rim</code><br/>Example: <code style={{color:'#aaa'}}>Toyota, RAV4, 2023, 235, 55, 19</code></div>
      <textarea value={bulkText} onChange={e=>setBulkText(e.target.value)} rows={8} style={{...inp,resize:'vertical',fontFamily:'monospace',fontSize:12,marginBottom:14}} placeholder={"Toyota, RAV4, 2023, 235, 55, 19\nFord, Ranger, 2023, 265, 60, 18\nSuzuki, Swift, 2023, 185, 60, 15"}/>
      <div style={{display:'flex',gap:10}}>
    ...
    <div style={{marginBottom:14}}><input placeholder="🔍  Search make, model, year…" value={search} onChange={e=>setSearch(e.target.value)} style={{...inp,maxWidth:320}}/></div>
        ...
        <div style={{background:'#141414',border:'1px solid #1e1e1e',borderRadius:4,overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style={{background:'#1a1a1a'}}>
              {['Model','Year','OE Size','Notes','Actions'].map(h=><th key={h} style={th}>{h}</th>)}
            </tr></thead>
            <tbody>
              {filtered.filter(v=>v.make===make).sort((a,b)=>a.model.localeCompare(b.model)||b.year-a.year).map((v,i)=>(
                <tr key={v.id} style={{borderTop:'1px solid #1e1e1e',background:i%2===0?'#161616':'#141414'}}>
                  ...
                  <td style={{...td,color:'#555',fontSize:12}}>{v.notes||'—'}</td>
    ...
    {deleteTarget&&<Modal onClose={()=>setDeleteTarget(null)}>
      <div style={{textAlign:'center'}}>
        ...
        <div style={{color:'#888',fontSize:14,marginBottom:24}}>{deleteTarget.make} {deleteTarget.model} {deleteTarget.year}<br/>{deleteTarget.width}/{deleteTarget.profile}R{deleteTarget.rim}<br/><span style={{color:'#ff6666'}}>Cannot be undone.</span></div>
...
function SettingsTab({ showToast }) {
  ...
  const Section=({title,children})=><div style={{background:'#161616',border:'1px solid #222',borderRadius:6,padding:24,marginBottom:16}}><div style={{...BB,fontSize:18,letterSpacing:3,color:O,marginBottom:18}}>{title}</div>{children}</div>
  const Field=({label,k,type='text',placeholder=''})=><div style={{marginBottom:14}}><label style={lbl}>{label}</label><input type={type} value={form[k]||''} onChange={e=>f(k,e.target.value)} style={inp} placeholder={placeholder}/></div>
  const TextArea=({label,k,rows=3,placeholder=''})=><div style={{marginBottom:14}}><label style={lbl}>{label}</label><textarea value={form[k]||''} onChange={e=>f(k,e.target.value)} rows={rows} style={{...inp,resize:'vertical'}} placeholder={placeholder}/></div>
  ...
  return <div style={{maxWidth:800}}>
    ...
    <Section title="BOOKING SYSTEM">
      <div style={{background:'#111',border:'1px solid #1a1a1a',borderRadius:4,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#666',lineHeight:1.7}}>
        Configure when customers can book. Block specific dates (public holidays) in the <strong style={{color:'#aaa'}}>Blocked Dates</strong> tab.
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
        <Field label="Opening Time" k="booking_open_time" placeholder="08:00"/>
        <Field label="Closing Time" k="booking_close_time" placeholder="17:00"/>
        <Field label="Slot Duration (minutes)" k="booking_slot_minutes" placeholder="60"/>
        <Field label="Buffer Between Bookings (minutes)" k="booking_buffer_minutes" placeholder="0"/>
      </div>
      <div style={{marginBottom:14}}>
        <label style={lbl}>Open Days (tick the days you take bookings)</label>
        <div style={{display:'flex',gap:16,flexWrap:'wrap',marginTop:4}}>
          {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((day,i)=>{
            const days=form.booking_open_days||'1111110'
            const open=days[i]==='1'
            return <label key={day} style={{display:'flex',alignItems:'center',gap:6,cursor:'pointer'}}>
              <input type="checkbox" checked={open} onChange={e=>{const d=[...days];d[i]=e.target.checked?'1':'0';f('booking_open_days',d.join(''))}} style={{accentColor:O,width:16,height:16}}/>
              <span style={{color:'#aaa',fontSize:14}}>{day}</span>
            </label>
          })}
        </div>
      </div>
    ...
    <Section title="HERO / LANDING PAGE">
      <div style={{background:'#111',border:'1px solid #1a1a1a',borderRadius:4,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#666'}}>Controls the large banner at the top of your shop page.</div>
      ...
      <TextArea label="Subtext (below heading)" k="hero_subtext"/>
      <Field label="Price match badge text" k="hero_badge_text"/>
      <div style={{marginBottom:14}}>
        <label style={lbl}>Shop Logo / Hero Image</label>
        <div style={{fontSize:11,color:'#555',marginBottom:8}}>Shown in the navbar and hero section. PNG with transparent background recommended.</div>
    ...
    <Section title="FOOTER &amp; ABOUT PAGE">
      <TextArea label="Footer tagline" k="footer_tagline"/>
      ...
      <div style={{marginBottom:14}}>
        ...
        <textarea value={form.maps_embed||''} onChange={e=>f('maps_embed',e.target.value)} rows={3} style={{...inp,resize:'vertical',fontSize:11}} placeholder='<iframe src="https://www.google.com/maps/embed?..." ...></iframe>'/>
      </div>
    </Section>
    ...
    <Section title="EMAIL NOTIFICATIONS (SMTP)">
      <div style={{fontSize:12,color:'#555',marginBottom:14}}>Fill in to receive email alerts and send confirmation emails to customers after bookings/enquiries.</div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
      ...
      <Field label="Notification Email (receives new booking/enquiry alerts)" k="notify_email" placeholder="you@email.com"/>
    </Section>
...
function AdminPanel({ showToast }) {
  ...
  const tabBtn=(t,label)=>(
    <button key={t} onClick={()=>{setTab(t);setEditMode(null);setShowBulk(false)}} style={{background:tab===t?O:'transparent',border:`1px solid ${tab===t?O:'#333'}`,color:tab===t?'#000':'#888',padding:'8px 14px',borderRadius:4,cursor:'pointer',fontWeight:800,fontSize:12,letterSpacing:2,fontFamily:"'Barlow Condensed',sans-serif"}}>{label}</button>
  )

  return <div style={{maxWidth:1360,margin:'0 auto',padding:'28px 20px'}}>
...
export default function App() {
  ...
  const [searchResult,setSearchResult] = useState(null) // {type:'vehicle'|'size', data, label}
  ...
  // Load tyres based on filters (only when no search result active)
  useEffect(()=>{
  ...
  const handleSizeSearch = async (size) => {
    ...
    try {
      ...
      setSearchResult({ type:'size', tyres:results, label:`Size: ${sizeLabel}` })
  ...
  return (
    <div style={{fontFamily:"'Barlow Condensed',Impact,sans-serif",background:'#0d0d0d',color:'#f0f0f0',minHeight:'100vh',paddingBottom:compareList.length>=2?150:0}}>
      ...
      {page==='shop'&&<>
        ...
        {searchResult&&<div style={{maxWidth:1360,margin:'16px auto 0',padding:'0 20px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'#1a1a1a',border:`1px solid ${O}44`,borderRadius:6,padding:'12px 18px'}}>
            <div style={{fontSize:13,color:'#aaa'}}>
              Showing results for <strong style={{color:O}}>{searchResult.label}</strong>
              <span style={{color:'#555',marginLeft:12}}>· {searchResult.tyres.length} tyre{searchResult.tyres.length!==1?'s':''} found</span>
            </div>
            <Btn variant="secondary" onClick={clearSearch} style={{padding:'5px 14px',fontSize:12}}>✕ Clear Search</Btn>
          </div>
        </div>}
          ...
          {loading?<div style={{textAlign:'center',color:'#444',padding:'80px 0'}}>Loading tyres…</div>
            :displayTyres.length===0
              ?<div style={{textAlign:'center',color:'#444',padding:'80px 0'}}>
                ...
                <div style={{marginTop:12}}>{searchResult?'No tyres found for this search.':'No tyres match your filters.'}</div>
                {searchResult&&<div style={{marginTop:16}}><Btn onClick={clearSearch}>Browse All Tyres</Btn></div>}
              </div>
