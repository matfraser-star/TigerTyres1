  ...
  return (
    <div style={{fontFamily:"'Barlow Condensed',Impact,sans-serif",background:'#0d0d0d',color:'#f0f0f0',minHeight:'100vh',paddingBottom:compareList.length>=2?150:0}}>
      ...
      {page==='shop'&&<>
        ...
        {/* Standard filters — only show when no search result */}
        {!searchResult&&<div style={{background:'#141414',borderBottom:'1px solid #1e1e1e',padding:'12px 20px',marginTop:16,position:'sticky',top:62,zIndex:800}}>
          <div style={{maxWidth:1360,margin:'0 auto',display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
            <input placeholder="🔍  Brand, model…" value={filters.search} onChange={e=>setFilters(f=>({...f,search:e.target.value}))} style={{...inp,flex:'1 1 180px',maxWidth:280}}/>
            ...
            <span style={{color:'#444',fontSize:11,whiteSpace:'nowrap'}}>{tyres.length} in stock</span>
          </div>
        </div>}
