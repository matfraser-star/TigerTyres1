  ...
  return <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: CARD, borderTop: `2px solid ${O}`, zIndex: 800, padding: '16px 20px', boxShadow: '0 -8px 40px rgba(0,0,0,0.4)', backdropFilter: 'blur(12px)' }}>
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div style={{ ...BB, fontSize: 16, color: TEXT }}>Comparing {tyres.length} tyres</div>
        <Btn variant="secondary" onClick={onClose} size="sm">Close</Btn>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr>
            <th style={{ padding: '6px 12px', textAlign: 'left', color: TEXT3, fontSize: 11, fontWeight: 600, width: 110 }}>SPEC</th>
            {tyres.map(t => (<th key={t.id} style={{ padding: '6px 12px', textAlign: 'left', minWidth: 170 }}><div style={{ color: O, fontSize: 12, fontWeight: 600 }}>{t.brand}</div><div style={{ fontWeight: 800, color: TEXT }}>{t.model}</div><button onClick={() => onRemove(t.id)} style={{ background: 'transparent', border: 'none', color: TEXT3, cursor: 'pointer', fontSize: 11, padding: 0 }}>remove ✕</button></th>))}
          </tr></thead>
          <tbody>{fields.map(([label, key]) => (<tr key={label} style={{ borderTop: `1px solid ${BORDER}` }}><td style={{ padding: '6px 12px', color: TEXT3, fontSize: 11 }}>{label}</td>{tyres.map(t => { const val = typeof key === 'function' ? key(t) : t[key]; return <td key={t.id} style={{ padding: '6px 12px', color: label === 'Price' ? O : TEXT, fontWeight: label === 'Price' ? 800 : 400, fontSize: label === 'Price' ? 17 : 13 }}>{val}</td> })}</tr>))}</tbody>
        </table>
      </div>
    </div>
  </div>
