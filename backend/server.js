...
app.post('/api/admin/vehicles', requireAuth, (req,res) => {
  ...
  if (!make||!model||!year||!width||!profile||!rim) return res.status(400).json({error:'All fields required'});
...
app.put('/api/admin/vehicles/:id', requireAuth, (req,res) => {
  const {make,model,year,width,profile,rim} = req.body;
  if (!make||!model||!year||!width||!profile||!rim) return res.status(400).json({error:'All fields required'});
  getDb().prepare('UPDATE vehicles SET make=?,model=?,year=?,width=?,profile=?,rim=? WHERE id=?').run(make,model,+year,+width,+profile,+rim,req.params.id);
...
// Bulk import vehicles from CSV-like array
app.post('/api/admin/vehicles/bulk', requireAuth, (req,res) => {
  ...
  const ins = getDb().prepare('INSERT OR IGNORE INTO vehicles (make,model,year,width,profile,rim) VALUES (?,?,?,?,?,?)');
  let added = 0;
  getDb().transaction(rows => { for(const v of rows){ ins.run(v.make,v.model,+v.year,+v.width,+v.profile,+v.rim); added++; } })(vehicles);
